...
(** [init_state b1 b2] returns the initial state of the game when playing
    battleship. It reads in two boards and creates a state consisting of 4
    boards, representing the two players' inner boards and outer boards. *)

val get_inner : t -> int -> Board.b
(** [get_inner state player] returns [player]'s inner board, which has
    information on where [player] placed their battleships*)

val get_outer : t -> int -> Board.b
(** [get_outer state player] returns [player]'s outer board, which has
    information on where the other player has attempted to hit*)
...
(**[move state player ship_name x y] returns a new state where player [player],
   ...
   [x] if [x] is positive and to the left by [x] if [x] is negative, and up by
   [y] if [y] is positive and down by [y] is [y] is negative.*)
